...
export class AgentBrowser {
  ...
  run(args: string[], options: { json?: boolean } = {}): {
    stdout: string;
    stderr: string;
    exitCode: number;
  ...
  ensureCursorOverlay(): void {
    if (!this.showCursor || this.cursorInjected) return;
  ...
  clickVisible(ref: string): void {
    if (config.showCursor) this.highlight(ref);
    this.click(ref);
  }
  ...
  moveOverlayCursor(x: number, y: number): void {
    this.evalScript(
      `window.dispatchEvent(new CustomEvent('qa-cursor-move', { detail: { x: ${x}, y: ${y} } }));`,
    );
  }
