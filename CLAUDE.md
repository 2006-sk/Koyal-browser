...
- **NEW robustness finding (repro'd 3×): the agent-browser daemon WEDGES on Koyal's heavy wizard** (video elements + multi-min processing) after ~1 flow — every subsequent flow aborts with `agent-browser timed out after 30000ms: open/get url`. Recurs even after a full `pkill -9` + stale-profile clean + fresh daemon. Root aggravator tonight: 19 orphaned agent-browser + 15 Chrome procs accumulated over the session. **autoqa needs daemon-recycle-on-repeated-timeout** (detect N consecutive subprocess timeouts → restart the session, re-auth, resume) — without it, one wedge kills the whole test phase. Likely needs a machine/Chrome restart to fully clear; do NOT keep retrying blindly.
